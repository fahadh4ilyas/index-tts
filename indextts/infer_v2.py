...
from indextts.utils.streamer import AudioStreamer, AsyncAudioStreamer
...
class IndexTTS2:
    ...
    def _init_step_input(self, spk_audio_prompt, text,
              emo_audio_prompt=None, emo_alpha=1.0,
              emo_vector=None,
              use_emo_text=False, emo_text=None, use_random=False, interval_silence=200,
              max_text_tokens_per_segment=120, more_segment_before=0, audio_streamer=None, stop_check_fn=None, **generation_kwargs):
        ...
        segments = self.tokenizer.split_segments(text_tokens_list, max_text_tokens_per_segment, quick_streaming_tokens = more_segment_before)
        ...
        return dict(
            ...
            inputs = dict(
                ...
                emovec_mat = emovec_mat if emo_vector is not None else None,
                weight_vector = weight_vector if emo_vector is not None else None,
    ...
    def _single_step_infer(
        ...
        emo_alpha: float,
        emo_vector: Optional[torch.Tensor],
        emovec_mat: Optional[torch.Tensor],
        weight_vector: Optional[torch.Tensor],
        top_p: float,
        top_k: int,
        temperature: float,
        ...
        with torch.no_grad():
            ...
            if audio_streamer is not None:
                audio_streamer.put(wav)
            if silence == None:
                silence = self.interval_silence([wav.cpu()], sampling_rate=sampling_rate, interval_silence=interval_silence)
            if audio_streamer is not None:
                audio_streamer.put(silence)
        ...
        return dict(
            ...
            inputs = dict(
                step = step,
                sent = segments[step] if step < len(segments) else None,
                segments = segments,
                spk_cond_emb = spk_cond_emb,
                emo_cond_emb = emo_cond_emb,
                emo_alpha = emo_alpha,
